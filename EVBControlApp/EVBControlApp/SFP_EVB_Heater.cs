...
{
    /// <summary>
    /// SFP EVB 加热台 TCP 控制类（WinForm专用版）
    /// </summary>
    public class SFP_EVB_Heater
    {
        ...
        #region 业务方法
        public bool IIC_Set(int slot, string deviceAddr, string regAddr, string dataLen, string data)
        {
            var res = SendCommand($"IIC{slot}:set {deviceAddr},{regAddr},{dataLen},{data}");
            return !string.IsNullOrEmpty(res) && res.Contains("iic set ok");
        }

        public string IIC_Get(int slot, string deviceAddr, string regAddr, string dataLen)
        {
            return SendCommand($"IIC{slot}:get {deviceAddr},{regAddr},{dataLen}");
        }
        ...
        {
            var res = SendCommand($"evb{slot}:setvoltage {voltage}");
            return !string.IsNullOrEmpty(res) && res.Contains("set Voltage ok!");
        }

        public string GetPowerEN(int slot = 1)
        {
            var res = SendCommand($"IO{slot}:getPowerEN");
            if (res == null || !res.Contains("POWER_EN:")) return null;
            return res.Split(':')[1].Trim();
        }

        public string GetTxDis(int slot = 1)
        {
            var res = SendCommand($"IO{slot}:getTxDis");
            if (res == null || !res.Contains("TX_DIS:")) return null;
            return res.Split(':')[1].Trim();
        }
        #endregion
    }
