...
namespace SFP模块终测检查软件
{
    ...
    //===========================================================================
        ...
        /// <summary>光源(LAC)位置信息</summary>
        public class LacLocation
        {
            public string Slot;   // OTP12槽位号，如 "13"
            public int Channel;   // LAC通道号 1或2
            public LacLocation(string slot, int ch) { Slot = slot; Channel = ch; }
        }
    ...
    public static class HardwareMap
    {
        ...
        /// <summary>
        /// 获取模块对应"接收端光源(LAC)"的位置
        /// 每个模块一对光源通道（与Rx VOA布局对应）：
        /// SLOT-13: ch1→模块1, ch2→模块2
        /// SLOT-14: ch1→模块3, ch2→模块4
        /// 如果实际硬件光源槽位不同，请修改此处映射。
        /// </summary>
        public static LacLocation GetLac(int channelIndex)
        {
            switch (channelIndex)
            {
                case 0: return new LacLocation("13", 1);
                case 1: return new LacLocation("13", 2);
                case 2: return new LacLocation("14", 1);
                case 3: return new LacLocation("14", 2);
                default: throw new ArgumentException("无效的通道索引: " + channelIndex);
            }
        }
