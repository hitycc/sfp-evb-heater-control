...
namespace SFPXFP自动测试软件多端口
{
    ...
    public class TestControl
    {
        ...
        public TestControl(byte dut_num)
        {
            ...
            GlobalVarFun.setup.rx_test = false;
        ...
        private async Task<bool> RxPwrDDMAutoCal_Async()
        {
            ...
            switch (Dut)
            {
                ...
                case 3:
                    ...
                    result.message = "设置TXSFP光源3：Success" + " AttVal" + DOA3.rxCalAtt[2].ToString() + " rxAdc:" + rxAdc[2].ToString();
                    break;
                case 4:
                    otp12.VOA_SetAttenuation(Dut, DOA4.rxCalAtt[2]);
                    result.message = "设置TXSFP光源3：Success" + " AttVal" + DOA4.rxCalAtt[2].ToString() + " rxAdc:" + rxAdc[2].ToString();
                    break;
            }
            opticaldoaatt.SetAttenuation(DOA.rxCalAtt[2]);
            rxAdc[2] = test.GetRxADC();
        ...
        private bool RxSenBitErrorCheck()
        {
            ...
            {
                case 1:
                    opticaldoaatt.SetAttenuation(DOA.rxOverLoadAtt); //饱和点
                    break;
                case 2:
                    opticaldoaatt.SetAttenuation(DOA2.rxOverLoadAtt); //饱和点
                    break;
            }
            ...
            {
                channel = pssChannel.Trim().Substring(pssChannel.Trim().Length - 1);//截取PSS通道号
                try
                {
                    pssbert.ClearChannelError(channel);//清除误码率  
                    Status = pssbert.GetChannelStatus(channel);
                    num25G = Convert.ToDouble(str25G);
                    if (Status.Contains("Y Y"))//误码
                    {
                        string fpn = "";
                        switch (Dut)
                        {
                        ...
                        if (fpn.Contains("T5-"))
                        {
                            result.message = "误码率 : " + Status;
                            ModListBoxShow(this, result);
                            numnew = Convert.ToDouble(Status.Substring(21, 11).Trim());//22,11
                            if (numnew < num25G)
                            {
                                result.message = "灵敏度测试PASS: " + Status;
                                ModListBoxShow(this, result);
                                //灵敏度测试PASS
                            }
                            else
                            {
                                result.message = "饱和光功率测试失败YY：" + "误码率 : " + Status;
                                ModListBoxShow(this, result);
                                errmsg += "饱和光功率测试失败YY：\r\n";//误码
                                errmsg += Status + "\r\n";//误码率
                            }
                        ...
                        else
                        {
                            ...
                            errmsg += Status + "\r\n";//误码率
                            result.message = "饱和光功率测试失败YY：" + errmsg;
                            ModListBoxShow(this, result);
                        }
                    }
                    else if (Status.Contains("Y N"))//同步
                    {
                        result.message = "饱和光功率测试：" + Status;
                        ModListBoxShow(this, result);
                        string fpn = "";
                        switch (Dut)
                        {
                        ...
                        if (fpn.Contains("T5-"))
                        {
                            numnew = Convert.ToDouble(Status.Substring(22, 10).Trim());//22,11
                            if (numnew <= num25G)
                            {
                                //灵敏度测试PASS
                            }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    errmsg += "Saturation light power test failed：\r\n";
                                }
                                errmsg += Status + "\r\n";//误码率
                            }
                        }
                    }
                    else if (Status.Contains("N Y"))//失步
                    {
                        result.message = "饱和光功率测试：" + Status;
                        ModListBoxShow(this, result);
                        if (GlobalVarFun.Language == "Chinese")
            ...
            {
                case 1:
                    opticaldoaatt.SetAttenuation(DOA.rxSenAtt);      //灵敏度点
                    break;
                case 2:
                    opticaldoaatt.SetAttenuation(DOA2.rxSenAtt);      //灵敏度点
                    break;
            }
            ...
            if (GlobalVarFun.setup.rx_sen_test)
            {
                ...
                {
                    case 1:
                        opticaldoaatt.SetAttenuation(DOA.rxSenAtt - 2);      //灵敏度点 
                        break;
                    case 2:
                        opticaldoaatt.SetAttenuation(DOA2.rxSenAtt - 2);      //灵敏度点 
                        break;
                }
                ...
                //opticaldoaatt.SetAttenuation(DOA.rxSenAtt - 4);      //灵敏度点
                //Thread.Sleep(500);//
                //errmsg += sencheck(channel);
                switch (Dut)
                {
                    case 1:
                        opticaldoaatt.SetAttenuation(DOA.rxOverLoadAtt);  //光饱和点
                        break;
                    case 2:
                        opticaldoaatt.SetAttenuation(DOA2.rxOverLoadAtt);  //光饱和点
                        break;
                }
                ...
                {
                    case 1:
                        opticaldoaatt.SetAttenuation(DOA.rxOverLoadAtt + 3);  //光饱和点
                        break;
                    case 2:
                        opticaldoaatt.SetAttenuation(DOA2.rxOverLoadAtt + 3);  //光饱和点
                        break;
                }
                pssbert.ClearChannelError(channel);//清除误码率     
                Thread.Sleep(500);//
        ...
        private async Task<bool> RxSenBitErrorCheck_Async()
        {
            ...
            // VOA初始化 - 参照RxPwrDDMAutoCal_Async模式
            string slotStr = VOArxDutToSlot[Dut];
            otp12.SetSlot(slotStr);
            otp12.VOA_SetAlcState(Dut, "OFF");
            otp12.VOA_SetMode(Dut, "ATTenuation");
            ...
            if (GlobalVarFun.setup.bert_connect && GlobalVarFun.setup.rx_sen_test)
            {
                try
                {
                    ...
                    Status = otp12.BERT_GetErrData(Dut);
                    try
                    {
        ...
        private async Task <bool> AutoTestRxAPD_Async()
        {
            double[] psspertbuf = new double[256];
            double valmin = psspertbuf[0];
            byte valminindex = 0;
        ...
        private bool EML_AutoTest(UInt16 emlvalmin, UInt16 emlvalmax)
        {
            ...
            string slotStr = EmlDutToOtpSlot[Dut];
            otp12.SetSlot(slotStr);
            otp12.SW_SetRouteForModule(Dut, true); // true=发射方向
            Thread.Sleep(500);
            // 按Dut选择目标波长
            ...
            else
            {
                do
                {
                    ...
                    result_err = wavelenth - wLengthTarget;
                    //
                    if (result_err < 0)
                    {
         ...
         IProgress<ReturnTxRxResult> progress)// 参数类型改为 IProgress<T>   
        {
            ...
            if (GlobalVarFun.setup.rx_apd_test)
            {
                ...
                await Task.Delay(waittimes);
                byte TX_EN_byte = i2c.TWI_ReadByte(0xa2, 0x79);
                TX_EN_byte = Bit.SetBit(TX_EN_byte, 1);
                i2c.TWI_WriteByte(0xa2, 0x79, TX_EN_byte);
                ...
                Task<bool> autoapd = AutoTestRxAPD_Async();
            ...
            if (GlobalVarFun.setup.rx_test)
            {
               ...
               // if (RxPwrDDMAutoCal() == false)
                ...
                //        progress.Report(new ReturnTxRxResult { Percentage = 100, StatusText = "模块接收校准 DDM精度检查失败, 请插入下一只模块......" });
                //    }
                //    else
                //    {
                //        AddTestLog("The received DDM detection accuracy is out of the set range!");
                //        retutntxrxresult.TestResultMessage = "Module failed to receive calibration DDM accuracy check, please insert next module......";
                //        progress.Report(new ReturnTxRxResult { Percentage = 100, StatusText = "Module failed to receive calibration DDM accuracy check, please insert next module......" });
                //    }
                //    //
                //    switch (Dut)
                //    {
                ...
                //Task<bool> rxloscheck = RxLosAlarmCheck_Async();
                //res = await rxloscheck;
                //if (res == false)
                ...
                //    if (GlobalVarFun.Language == "Chinese")
                //    {
                //        AddTestLog("LOS功能检查失败！");
                //        retutntxrxresult.TestResultMessage = "模块接收LOS或告警功能 检查失败, 请插入下一只模块......";
                //        progress.Report(new ReturnTxRxResult { Percentage = 100, StatusText = "模块接收LOS或告警功能 检查失败, 请插入下一只模块......" });
                //    }
                //    else
                //    {
                //        AddTestLog("LOS function check failed!");
                ...
                //        progress.Report(new ReturnTxRxResult { Percentage = 100, StatusText = "The module fails to receive the LOS or alarm function check. Please insert the next module......" });
                //    }
                //    switch (Dut)
                //    {
                //        case 1:
            ...
            if (GlobalVarFun.setup.tx_test)
            {
                ...
                //Task<bool> testtxPwrauto = TxPowerAutoSet_Async();
                //bool res = await testtxPwrauto;
                //if (res == false)
                ...
                if (GlobalVarFun.setup.algorithm_25g_lr || GlobalVarFun.setup.tx_eml_test)
                {
                    ...
                    {
                        case 1:
                            TestResult.txPower = opticalmeter.ReadPower(meter_ch, GlobalVarFun.setup.meter_delay) + meter_err;//   
                            break;
                        case 2:
                            TestResult2.txPower = opticalmeter.ReadPower(meter_ch, GlobalVarFun.setup.meter_delay) + meter_err;//   
                            break;
                        default:
                            break;
                    }
                    ...
                    //}
                    //获取 DDM TXPower
                    //if (TxPWR_textBox.Text == "")
                    //{
                    //    retutntxrxresult.ErrorMessage += "Tx二次校准失败,获取 TxDDM 错误";
                    //    return false;
                    //}
                    //TestResult.txPowerDDM = (float)Convert.ToDouble(TxPWR_textBox.Text);//(float)test.GetTxPwr();               
                    //if ((TestResult.txPowerDDM > TestSet.txPwr_Max) || (TestResult.txPowerDDM < TestSet.txPwr_Min))
                    //{
                    //    AddTestLog("发射光功率超出设定范围：" + TestResult.txPowerDDM.ToString("0.00"));
                    //    //return false;
                    //}
                }
                // 写入发射校准参数到模块
                ...
                //Task<bool> txfinalcheck = TxFinalTestCheck_Async(false);
                //res = await txfinalcheck;
                //// if (TxFinalTestCheck(false) == false)//true
                //if (res == false)
                //{
                //    if (GlobalVarFun.Language == "Chinese")
                //    {
                ...
                //        retutntxrxresult.TestResultMessage = "Module transmit optical power and extinction ratio check failed, please insert the next module......";
                //        progress.Report(new ReturnTxRxResult { Percentage = 100, StatusText = retutntxrxresult.TestResultMessage });
                //    }
                //    switch (Dut)
                //    {
        ...
        CHECK_POS:
            ...
            //
            // 4、保存参数到数据库
            if (((GlobalVarFun.sql_connect_status == true) && (GlobalVarFun.record_need_save == true)) || ((GlobalVarFun.sql_connect_status_2 == true) && (GlobalVarFun.record_need_save_2 == true)))
            {
                AddTestLog("保存参数到数据库");
        ...
        IProgress<ReturnTxRxResult> progress)// 参数类型改为 IProgress<T> 
        {
            ...
            //
            //发射波长检测
            if (GlobalVarFun.setup.tx_eml_test && GlobalVarFun.setup.kt86120x_connect)
            {
                AddTestLog("发射波长检测");
                await Task.Delay(waittimes);
            ...
            if ((GlobalVarFun.sql_connect_status == true) && (GlobalVarFun.record_need_save == true))
            {
                ...
                //    if (GlobalVarFun.Language == "Chinese")
                //    {
                //         retutntxrxresult.TestResultMessage = "模块终测参数OK，未保存到SQL数据库，请检查数据库连接！！请插入下一只模块......";
                //        AddTestLog("SQL数据库写入终测记录进程被占用，终测参数未保存！");
                //    }
                //    else
                //    {
                //         retutntxrxresult.TestResultMessage = "Module final test parameters OK, not saved to SQL database, please check the database connection!! Please insert the next module......";//模块终测参数OK，未保存到SQL数据库，请检查数据库连接！！请插入下一只模块
                //        AddTestLog("SQL database write final test record process is occupied, final test parameters are not saved!");
                //    }
                //    return false;
                //}
                //// 启动后台进程 保存测试数据到数据库
