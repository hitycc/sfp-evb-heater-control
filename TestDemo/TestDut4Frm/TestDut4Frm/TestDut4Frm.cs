...
namespace SFPXFP自动测试软件多端口
{
    public partial class TestDut4Frm : Form
    {
        ...
        {
            testcontrol4.Read_moduleInfo();
            testcontrol4.Converted_analog_values();
            testcontrol4.Read_AlarmWarn_Thresholds();
            //await Task.Run(() =>
            //{
            ...
            this.BeginInvoke(new MethodInvoker(() =>
            {
                ...
                SetLED(temp_ha_pictureBox, TestResult4.tempHA_flag);
                SetLED(temp_la_pictureBox, TestResult4.tempLA_flag);
                SetLED(vcc_ha_pictureBox, TestResult4.vccHA_flag);
                SetLED(vcc_la_pictureBox, TestResult4.vccLA_flag);
                SetLED(bias_ha_pictureBox, TestResult4.txBiasHA_flag);
                SetLED(bias_la_pictureBox, TestResult4.txBiasLA_flag);
                SetLED(txpwr_ha_pictureBox, TestResult4.txPwrHA_flag);
                SetLED(txpwr_la_pictureBox, TestResult4.txPwrLA_flag);
                SetLED(rxpwr_ha_pictureBox, TestResult4.rxPwrHA_flag);
                SetLED(rxpwr_la_pictureBox, TestResult4.rxPwrLA_flag);

                SetLED(temp_hw_pictureBox, TestResult4.tempHW_flag);
                SetLED(temp_lw_pictureBox, TestResult4.tempLW_flag);
                SetLED(vcc_hw_pictureBox, TestResult4.vccHW_flag);
                SetLED(vcc_lw_pictureBox, TestResult4.vccLW_flag);
                SetLED(bias_hw_pictureBox, TestResult4.txBiasHW_flag);
                SetLED(bias_lw_pictureBox, TestResult4.txBiasLW_flag);
                SetLED(txpwr_hw_pictureBox, TestResult4.txPwrHW_flag);
                SetLED(txpwr_lw_pictureBox, TestResult4.txPwrLW_flag);
                SetLED(rxpwr_hw_pictureBox, TestResult4.rxPwrHW_flag);
                SetLED(rxpwr_lw_pictureBox, TestResult4.rxPwrLW_flag);
            }));
        }
        #endregion
        ...
        private void AddTestLog(string strMessage)
        {
            ...
            GlobalVarFun.mycontrol_dut4.ModListBoxShow(this, result);
        ...
        private void start_button_Click(object sender, EventArgs e)
        {
           ...
           // GlobalVarFun.mycontrol_dut4.GetTypeDebugInfoFromAccessdb();
            ...
            {
                if ((TestResult4.waveforms_count >= 100) && (GlobalVarFun.eyeMaskIsOpened == false) && (GlobalVarFun.setup.dca_86100d) && (GlobalVarFun.setup.dca_n1092x == false))
                {
                    if (GlobalVarFun.Language == "Chinese")
                    {
                    ...
                    {
                        MessageBox.Show("The eye map template is not open, the eye map accumulation point test cannot be performed, please reconnect 86100! Please confirm!!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    return;
                }
            }
            ...
            if (autoTestCtrl == false)
            {
                ...
                {
                    if ((GlobalVarFun.setup.doa_connect4 == false) || (GlobalVarFun.setup.meter_connect == false)) // 光衰减器 和 光功率计
                    {
                        if (GlobalVarFun.Language == "Chinese")
                        {
                    ...
                    //2024.05.29
                    if (GlobalVarFun.setup.rx_sen_test)
                    {
                ...
                {
                    if (GlobalVarFun.setup.dca_connect == false) // 连接眼图仪判断
                    {
                        //2023.3.1修改
                        if (GlobalVarFun.testType == "firstTest")
                        {
                            ...
                            {
                                MessageBox.Show("Please connect the eye chart first! Please confirm!!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            }
                            return;
                        }
                        else
                        {
                            ...
                            {
                                MessageBox.Show("Final test: The emission only tests the luminous power, not the emission eye chart parameter test!!!", "prompt message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                    if (GlobalVarFun.setup.meter_connect == false && GlobalVarFun.setup.tx_use_dca_txpwr == false) // 不使用眼图仪测试发光功率时，连接光功率计判断
                    {
                        if (GlobalVarFun.Language == "Chinese")
                        {
                        ...
                        {
                            MessageBox.Show("Please connect the optical power meter first! Please confirm!!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
                        return;
                    }
                    //2023.3.1修改
                    if (GlobalVarFun.setup.tx_use_dca_txpwr == true && GlobalVarFun.setup.dca_connect == false)
                    {
                        if (GlobalVarFun.Language == "Chinese")
                        {
                        ...
                        {
                            MessageBox.Show("To test the luminous power with the eye graph, please connect the eye graph first! Please confirm!!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
                        return;
                    }
                }
        ...
        {
            if (GlobalVarFun.i2c_can_use_4 == false)
            {
                return;
            }
            ...
            Update();
            //
            //if (testcontrol4.GetVcc() < 2.0) //电源电压小于2.00V 异常模块
            if(TestResult4.vccDDM < 2.0)
            {
                ...
                Startautoset_button.BackColor = Color.Orange; //2018.8.2 add
                return;
            }
            ...
            {
                if (testcontrol4.CheckTestTypeInfo() == false) // 模块方案类型信息判断
                {
            ...
            ShowModuleDdmInfo();
            Update();
            if (TestResult4.fibertop_pn != "SFP-SM31TG-10DIU")//型号屏蔽，定制型号隐藏版本信息，跳过版本检查
            {
                if (testcontrol4.ShowCheckModuleStatus() == false) //显示并判断模块方案/速率/版本/工作状态等信息
                {
            ...
            if ((moduleOnline == false) && (GlobalVarFun.mycontrol_dut4.CheckDebugPWD() == 0x02)) // 检测到 新测试模块插入测试板
            {
                //初测 终测
        ...
        private async Task RunTestSequenceAsync(CancellationToken cancellationToken)
        {
            ...
            if (GlobalVarFun.testType == "firstTest")
            {
                ...
                //bool firstTestResult = await  testcontrol4.FirstTestProcessAsync(uiUpdateProgressForRxPwr as IProgress<ReturnTxRxResult>);
                if (firstTestResult)
                {
            ...
            else // (GlobalVarFun.testType == "finalTest")
            {
                ...
                //bool finalTestResult = await testcontrol4.FinalTestProcessAsync(uiUpdateProgressForRxPwr as IProgress<ReturnTxRxResult>);
                if (finalTestResult)
                {
                    TestResult4.Test_ok = true;
                    TestResult4.test_status = 2;
        ...
        private void UpdateUIControls(ReturnTxRxResult result)
        {
            ...
            this.BeginInvoke(new MethodInvoker(() =>
            {
                ...
                SetLED(sr850_pictureBox1, TestResult4.moduleIsSR);
                SetLED(chipok_pictureBox1, TestResult4.chipIsOK);
                //sr850_pictureBox1.Image = imageList1.Images["LedNone.ico"];
                //chipok_pictureBox1.Image = imageList1.Images["LedNone.ico"];
                SetLED(typeok_pictureBox1, false);
        ...
        private async void  StartNewTestSequence()
        {
            ...
            pnshow_textBox.Text = TestResult4.fibertop_pn;
            SetLED(i2cok_pictureBox1, false);
            SetLED(i2cok_pictureBox2, false);
            ...
            //        else
            //        {
            //            Startautoset_button.Text = "The model of the module to be tested is different from that customized by the customer ......";
            //        }
            //        goto RTN_POS; //return;
            //    }
            //    Refresh();
            ...
            //else // 飞思卓型号
            //{
            //    if ((pn_textBox.Text).Trim() != (moduletype_comboBox.Text).Trim())
            //    {
            //        Startautoset_button.BackColor = Color.Red;
            //        if (GlobalVarFun.Language == "Chinese")
            //        {
            ...
            //        else
            //        {
            //            Startautoset_button.Text = "The model of the module to be tested is different from that selected. Therefore, the module cannot be tested ......";
            //        }
            //        goto RTN_POS;  //return;
            //    }
            //}
            ...
            //txpower_textBox.Text = TestSet4.txPwr_target.ToString("F1") + "/test";
            //er_textBox.Text = TestSet4.txEr_target.ToString("F1") + "/test";
            //Bias_textBox_cal.Text = TestSet4.txBias_target.ToString("F1") + "/test";
            ...
            //label_APDval.Text = "test";
            //label_VON.Text = "test";
            //label_TosaTemp.Text = "test";
            //
            Startautoset_button.BackColor = Color.Honeydew;
            if (GlobalVarFun.Language == "Chinese")
            {
            ...
            finally
            {
                ...
                this.BeginInvoke(new MethodInvoker(() =>
                {
                    writeMyFileTxt("TestNum4", testnum.ToString());//模块插拔次数记录更新
                }));
        ...
        private void TestDut4Frm_Load(object sender, EventArgs e)
        {
            ...
            timer1.Start();
            timer = new Stopwatch();
            readMyFileTxt("TestNum4");//读取插拔模块次数记录
            tBNum.Text = testnum.ToString();
        ...
        {
            testcontrol4.sqlserver.SaveRecordToSQL(TestSet4.Dut);
        }
