...
namespace SFP模块终测检查软件
{
    public partial class Main_Form : Form
    {
       ...
       // I2C Ui2c;
        ...
        // 4通道状态面板控件数组（在构造函数中映射到设计器创建的控件）
        // 初始化为长度4的空数组，避免null引用异常；控件引用在设计器创建后通过数组映射赋值
        private Panel[] _channelPanels = new Panel[4];
        private Label[] _channelLabels = new Label[4];
        private Label[] _channelStatusLabels = new Label[4];
        private Label[] _channelStepLabels = new Label[4];
       ...
       // bool testDataIsOK = false;
        ...
        public Main_Form()
        {
            ...
            // ====== 映射设计器创建的4通道Panel和Label（公司规范：全部在VS设计器中创建） ======
            // 设计器中创建的 panel1~panel4 对应 CH0~CH3
            // 每个Panel内有3个Label：lblCh{X}Name(通道名)、lblCh{X}Status(状态)、lblCh{X}Step(步骤信息)
            _channelPanels[0] = panelCh0;
            _channelPanels[1] = panelCh1;
            _channelPanels[2] = panelCh2;
            _channelPanels[3] = panelCh3;

            _channelLabels[0] = lblCh0Name;
            _channelLabels[1] = lblCh1Name;
            _channelLabels[2] = lblCh2Name;
            _channelLabels[3] = lblCh3Name;

            _channelStatusLabels[0] = lblCh0Status;
            _channelStatusLabels[1] = lblCh1Status;
            _channelStatusLabels[2] = lblCh2Status;
            _channelStatusLabels[3] = lblCh3Status;

            _channelStepLabels[0] = lblCh0Step;
            _channelStepLabels[1] = lblCh1Step;
            _channelStepLabels[2] = lblCh2Step;
            _channelStepLabels[3] = lblCh3Step;
            // ====== 通道控件映射结束 ======
        ...
        {
            //初始化后台代理
            InitializeBackgoundWorker();// ① 初始化后台工作线程(旧的单通道BackgroundWorker)
        ...
        private void start_button_Click(object sender, EventArgs e)
        {
            ...
            if (autoTestCtrl == false)
            {
                ...
                {
                    // SFP初测模式：使用ERM消光比仪替代DCA眼图仪，不需要连接眼图仪
                    bool isSfpFirstTest = (GlobalVarFun.moduleType != "QSFP" && GlobalVarFun.testType == "firstTest");
                    ...
                    if (GlobalVarFun.instrument_connected == false) // 眼图仪连接检查
                    {
                        ...
                        if (GlobalVarFun.testType == "firstTest")
                        {
                            if (isSfpFirstTest)
                            {
                                // SFP初测：ERM替代DCA，提示但不阻止
                                MessageBox.Show("SFP初测模式：使用ERM消光比仪替代DCA眼图仪，不连接眼图仪也可测试！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            else
                            {
       ...
       // {
        ...
        {
            byte ApdValue = 254;            // APD初始值、ADC缓存、次数配置
            UInt16 rxAdc = 0;
            UInt16 lastRxAdc = 0;           // 保存上一次ADC采样值
            int maxTry = 35;
            int tryCount = 0;
            const int AdcJumpThreshold = 7; // ADC突变阈值：两次采样差值绝对值大于该值判定为突变
            byte stableApdVal = ApdValue;   // 记录未突变的稳定APD
            bool adcJumpDetected = false;   // 本通道是否检测到ADC跳变

            test.SetAPD(ApdValue);          // 首次设置初始APD并读取第一笔ADC

            Thread.Sleep(1000);

            lastRxAdc = test.GetRxADC();
            stableApdVal = ApdValue;
            AddTestLog($"通道{TestSet.ch} 初始APD={ApdValue}，ADC采样={lastRxAdc}");

            while (tryCount < maxTry)
            {
                tryCount++;
                ApdValue -= 5; // APD递减
                test.SetAPD(ApdValue);

                Thread.Sleep(500);

                rxAdc = test.GetRxADC();

                AddTestLog($"通道{TestSet.ch} 第{tryCount}次 APD={ApdValue}，当前ADC={rxAdc}，上一次ADC={lastRxAdc}");

                //计算两次ADC差值，判断是否突变
                int adcDiff = Math.Abs(rxAdc - lastRxAdc);
                if (adcDiff >= AdcJumpThreshold)
                {
                    AddTestLog($"通道{TestSet.ch} 检测到ADC突变！差值={adcDiff}，突变APD={ApdValue}，突变ADC={rxAdc}");
                    adcJumpDetected = true;

                    // 回退到上一步稳定APD并重新设置
                    stableApdVal -= 10; //通过数据分析，大多需要再减一些，暂时减10.

                    test.SetAPD(stableApdVal);
                    TestResult.rxapdVal = stableApdVal;
                    AddTestLog($"通道{TestSet.ch} 回退至稳定APD={stableApdVal}，完成本通道调节");
                    break; // 跳出APD递减循环，处理下一路通道
                }
            ...
            {
                case 0:
                    lbAPDValCh0.Text = stableApdVal.ToString();
                    break;
                case 1:
                    lbAPDValCh1.Text = stableApdVal.ToString();
                    break;
                case 2:
                    lbAPDValCh2.Text = stableApdVal.ToString();
                    break;
                case 3:
                    lbAPDValCh3.Text = stableApdVal.ToString();
                    break;
                default:
                    break;
            }
            ...
            if (!adcJumpDetected)
            {
                AddTestLog($"通道{TestSet.ch} 已达到最大调节次数{maxTry}，全程无ADC大幅跳变，最终稳定APD={stableApdVal}");
                return false;
            }
        ...
        //===================================================================
        // 更新4通道状态面板（在timer1_Tick中调用）
        //===================================================================
        private void UpdateChannelStatusPanel()
        {
            if (_channelTesters == null) return;

            for (int i = 0; i < 4; i++)
            {
                if (i >= _channelTesters.Length) break;
                if (_channelPanels[i] == null) continue;

                var tester = _channelTesters[i];
                ...
                if (tester.IsRunning && ctx.IsTesting)
                {
                    // 测试中 - 蓝色
                    panelColor = Color.LightSkyBlue;
                    nameColor = Color.DodgerBlue;
                    statusText = "测试中";
                    statusForeColor = Color.DarkBlue;
                    stepText = "Step: CH" + ctx.CurrentLane + " 正在测试...";
                    stepForeColor = Color.DimGray;
                }
                else if (ctx.TestPassed)
                {
                    // 测试通过 - 绿色
                    panelColor = Color.LightGreen;
                    nameColor = Color.SeaGreen;
                    statusText = "PASS";
                    statusForeColor = Color.DarkGreen;
                    stepText = "测试通过";
                    stepForeColor = Color.DarkGreen;
                }
                else if (!string.IsNullOrEmpty(ctx.ErrorMessage))
                {
                    // 测试失败 - 红色
                    panelColor = Color.LightCoral;
                    nameColor = Color.Firebrick;
                    statusText = "FAIL";
                    statusForeColor = Color.DarkRed;
                    stepText = ctx.ErrorMessage.Length > 25 ? ctx.ErrorMessage.Substring(0, 25) + "..." : ctx.ErrorMessage;
                    stepForeColor = Color.DarkRed;
                }
                else if (!ctx.ModuleOnline)
                {
                    // 无模块 - 橙色
                    panelColor = Color.LightGoldenrodYellow;
                    nameColor = Color.DarkOrange;
                    statusText = "等待模块";
                    statusForeColor = Color.DarkOrange;
                    stepText = "请插入模块...";
                    stepForeColor = Color.DimGray;
                }
                else
                ...
                // 统一更新颜色，确保子Label背景色跟随Panel
                _channelPanels[i].BackColor = panelColor;
                _channelLabels[i].BackColor = nameColor;
                _channelStatusLabels[i].BackColor = panelColor;
                _channelStatusLabels[i].Text = statusText;
                _channelStatusLabels[i].ForeColor = statusForeColor;
                _channelStepLabels[i].BackColor = panelColor;
                _channelStepLabels[i].Text = stepText;
                _channelStepLabels[i].ForeColor = stepForeColor;
            }
        }
