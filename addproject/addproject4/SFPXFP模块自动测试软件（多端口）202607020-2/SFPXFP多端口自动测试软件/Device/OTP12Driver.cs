...
namespace FibertopTest_Common
{
    ...
    public class OTP12Driver
    {
        ...
        private string SendScpiCmd(string cmd, int delayMs = 150)
        {
            lock (_lock)
            {
                if (!IsConnected) return null;
        ...
        #region 16 多槽位便捷方法（用于多线程并发控制不同槽位模块）
        /// <summary>
        /// 向指定槽位板卡的VOA通道设置衰减
        /// </summary>
        /// <param name="boardSlot">板卡槽位号 如"06"~"13"</param>
        /// <param name="ch">VOA通道1/2</param>
        /// <param name="attDb">衰减值(dB)</param>
        public bool VOA_SetAttenuationToSlot(string boardSlot, int ch, double attDb)
        {
            string cmd = $":INPut{ch}:ATT {attDb} DB";
            string res = SendScpiToSlot(boardSlot, cmd);
            return res != null && res.Contains("Command execute successfully");
        }
        ...
        /// 从指定槽位板卡的OPM通道读取光功率
        /// </summary>
        /// <param name="boardSlot">板卡槽位号 如"05"~"16"</param>
        /// <param name="ch">OPM通道1~6</param>
        public string OPM_ReadPowerFromSlot(string boardSlot, int ch)
        {
            return SendScpiToSlot(boardSlot, $":READ{ch}:SCALar:POWer:DC?");
        }

        /// <summary>
        /// 向指定槽位的LAC光源设置开关状态
        /// </summary>
        public bool LAC_SetStateToSlot(string boardSlot, int ch, string state)
        {
            string res = SendScpiToSlot(boardSlot, $":LAC{ch}:STATe {state}");
            return res != null && res.Contains("Command execute successfully");
        }
        ...
        /// 向指定槽位的LAC光源设置功率
        /// </summary>
        public bool LAC_SetPowerToSlot(string boardSlot, int ch, double powerDbm)
        {
            string res = SendScpiToSlot(boardSlot, $":LAC{ch}:POWer {powerDbm}");
            return res != null && res.Contains("Command execute successfully");
        }
        ...
        /// 向指定槽位的LAC光源设置波长
        /// </summary>
        public bool LAC_SetWaveToSlot(string boardSlot, int ch, int waveNm)
        {
            string res = SendScpiToSlot(boardSlot, $":LAC{ch}:WAVelength {waveNm}");
            return res != null && res.Contains("Command execute successfully");
        }

        /// <summary>
        /// 从指定槽位的VOA读取输入光功率（用于Rx端检测入光功率）
        /// </summary>
        public string VOA_GetInputPowerFromSlot(string boardSlot, int ch)
        {
            return SendScpiToSlot(boardSlot, $":INPut{ch}:POWer?");
        }

        /// <summary>
        /// 向指定槽位的光开关切换路由
        /// </summary>
        public bool SW_SetChannelToSlot(string boardSlot, int inCh, int outCh)
        {
            string res = SendScpiToSlot(boardSlot, $":ROUTe{inCh}:SCAN {outCh}");
            return res != null && res.Contains("Command execute successfully");
        }
        #endregion
    }
