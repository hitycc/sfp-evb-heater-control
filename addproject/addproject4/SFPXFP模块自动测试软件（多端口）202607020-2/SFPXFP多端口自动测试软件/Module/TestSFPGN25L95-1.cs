...
{
    public class SFPGN25L95 : ModuleTest
    {
        I2C i2c;
        public static byte Dut_num = 0;
        UInt16[] modlut = new UInt16[64];
        byte[] apclut = new byte[64];
        byte[] apdlut = new byte[64];

        byte[] register = new byte[62];
        ...
        public void Init(I2C i2c, byte Dut)
        {
            this.i2c = i2c;
            Dut_num = Dut;
            ...
            byte_debug_pwd[2] = 0x54;
            byte_debug_pwd[3] = 0x50;
        ...
        public bool SetDebugPWD()
        {
            ...
            //
            return Bit.ByteEquals(readbuffer, pwd_read); // GN25L95 密码位置读取固定为 0x00
        }
        ...
        public bool GetFlashInfoDebug()
        {
            ...
            if (SelectTable(1) == false) return false; // 表选择
            if (i2c.TWI_ReadPage(0xa2, 0xF8, readbuffer, 8) != 8)
            {
                return false;
            }
            switch (Dut_num)
            {
                ...
                case 2:
                    for (i = 0; i < 8; i++)
                    {
                        TestResult2.flash_data[i + 256 + 248] = readbuffer[i]; //0xF8-FF
                    }
                    break;
                default:
                    break;
            ...
            if (SelectTable(2) == false) return false; // 表选择
            ...
            switch (Dut_num)
            {
                ...
                case 2:
                    ...
                    // 更新 Fsn  飞思卓产品内部流水号
                    iFsn += TestResult2.flash_data[768 + 0]; // 0x00 = 1
                    iFsn <<= 8;
                    iFsn += TestResult2.flash_data[768 + 1]; // 0x01 = 2
                    iFsn <<= 8;
                    iFsn += TestResult2.flash_data[768 + 2]; // 0x02 = 3
                    iFsn <<= 8;
                    iFsn += TestResult2.flash_data[768 + 3]; // 0x03 = 4
                    iFsn <<= 8;
                    iFsn += TestResult2.flash_data[768 + 4]; // 0x04 = 5
        ...
        {
            UInt16[] modlut_read = new UInt16[64];
            byte[] apclut_read = new byte[64];
            byte[] apdlut_read = new byte[64];

            byte[] regcfg = new byte[62];
            byte[] regcfg_read = new byte[62];
            ...
            switch (Dut_num)
            {
                case 1:
                    for (i = 0; i < 64; i++)
                    {
                        modlut_read[i] = TestResult.flash_data[i * 2 + 640];
                        modlut_read[i] <<= 2;
                        temp_ui = TestResult.flash_data[i * 2 + 1 + 640];
                        temp_ui >>= 6;
                    ...
                    for (i = 0; i < 62; i++)
                    {
                        regcfg_read[i] = TestResult.flash_data[i + 512 + 160 - 128]; //0xA0 = 160 - 0x80
                    }
                    break;
                case 2:
                    for (i = 0; i < 64; i++)
                    {
                        modlut_read[i] = TestResult2.flash_data[i * 2 + 640];
                        modlut_read[i] <<= 2;
                        temp_ui = TestResult2.flash_data[i * 2 + 1 + 640];
                        temp_ui >>= 6;
                        modlut_read[i] += temp_ui;
                        //
                        apclut_read[i] = TestResult2.flash_data[i + 896];
                        apdlut_read[i] = TestResult2.flash_data[i + 960];
                    }

                    for (i = 0; i < 8; i++)
                    {
                        awflag_en_read[i] = TestResult2.flash_data[i + 256 + 248]; //0xF8-FF
                    }

                    for (i = 0; i < 62; i++)
                    {
                        regcfg_read[i] = TestResult2.flash_data[i + 512 + 160 - 128]; //0xA0 = 160 - 0x80
                    }
                    break;
                default:
                    break;
            ...
            /////////////////////////////////////////////////////////////////
            regcfg[45] = byte_debug_pwd[0]; // PW2
            regcfg[46] = byte_debug_pwd[1];
            regcfg[47] = byte_debug_pwd[2];
            regcfg[48] = byte_debug_pwd[3];

            regcfg[8] = regcfg_read[8] = 0x00; // APC SET
            regcfg[9] = regcfg_read[9] = 0x00; // BIAS SET
            regcfg[10] = regcfg_read[10] = 0x00; // BIAS SET
            regcfg[11] = regcfg_read[11] = 0x00; // MOD SET
            regcfg[12] = regcfg_read[12] = 0x00; // MOD SET

            regcfg[23] = regcfg_read[23] = 0x00; // ROGUE_ONU_MON
            regcfg[28] = regcfg_read[28] = 0x00; // LOS SET
            regcfg[30] = regcfg_read[30] = 0x00; // APD SET

            regcfg[49] = regcfg_read[49] = 0x00; // 保留字节 209
            /////////////////////////////////////////////////////////////////
            ...
            // check GN25L95 配置表 0xA0开始  62个字节
        ...
        public bool SetTxApcBias(UInt16 setVal)
        {
            ...
            // 选择表 02
            if (SelectTable(2) == false) return false;

            bool rtnVal = i2c.TWI_WriteByte(0xA2, 0xA8, (byte)setVal);
            Thread.Sleep(60); // 延时
            return rtnVal;
        }
        ...
        public bool SetTxModBias(UInt16 setVal)
        {
            ...
            if (i2c.TWI_WriteByte(0xA2, 0xAB, (byte)(setVal >> 2)) == false) return false;
            if (i2c.TWI_WriteByte(0xA2, 0xAC, (byte)(setVal << 6)) == false) return false;
            Thread.Sleep(60); // 延时
            return true;
        }
        ...
        public bool SetRxLos(UInt16 setVal)
        {
            ...
            bool rtnVal = i2c.TWI_WriteByte(0xA2, 0xBC, (byte)setVal);
            Thread.Sleep(60); // 延时
            return rtnVal;
        }
        ...
        public UInt16 GetRxADC()
        {
            ...
            // 选择表 02
            if (SelectTable(2) == false) return 0;

            if (i2c.TWI_ReadPage(0xA2, 0xF0, readbuffer, 2) == 2)
            {
                rxadc = readbuffer[0];
                rxadc <<= 8;
                rxadc += readbuffer[1];
        ...
        public UInt16 GetTxADC()
        {
            ...
            if (i2c.TWI_ReadPage(0xA2, 0xEE, readbuffer, 2) == 2)
            {
                txadc = readbuffer[0];
                txadc <<= 8;
                txadc += readbuffer[1];
        ...
        public bool WriteTxCalData()
        {
            ...
            if (i2c.TWI_WritePage(0xA2, 0xDE, writeByte, 4) != 4)
            {
                return false;
            }
            ...
            if (i2c.TWI_ReadPage(0xA2, 0xDE, readByte, 4) != 4)
            {
                return false;
            }
        ...
        public bool WriteRxCalData()
        {
            ...
            //
            if (i2c.TWI_WritePage(0xA2, 0xE2, writeByte, 6) != 6)
            {
                return false;
            }

            if (i2c.TWI_ReadPage(0xA2, 0xE2, readByte, 6) != 6)
            {
                return false;
            }
        ...
        public bool SaveTxDataAfterDebug()
        {
            ...
            temp_index = (int)(((float)(temp_value + 40)) / 2.5f);

            if (temp_index < 0) temp_index = 0;
            if (temp_index > 63) temp_index = 63;
            ...
            for (i = 0; i < 64; i++)
            {
                ...
                mod[i * 2] = (byte)(mod_value >> 2);
                mod[i * 2 + 1] = (byte)(mod_value << 6);
            }

            if (i2c.TWI_WritePage(0xa2, 0x00, A2hLow96, 96) != 96) return false;
            Thread.Sleep(10);
            if (i2c.TWI_ReadPage(0xa2, 0x00, A2hLow96_read, 96) != 96) return false;

            if (SelectTable(6) == false) return false; //表选择
            if (i2c.TWI_WritePage(0xa2, 0x80, apc, 64) != 64) return false;
            Thread.Sleep(10);
            if (i2c.TWI_ReadPage(0xa2, 0x80, apc_read, 64) != 64) return false;

            if (SelectTable(4) == false) return false; //表选择
            if (i2c.TWI_WritePage(0xa2, 0x80, mod, 128) != 128) return false;
            Thread.Sleep(10);
            if (i2c.TWI_ReadPage(0xa2, 0x80, mod_read, 128) != 128) return false;
        ...
        public bool InitModule()
        {
            ...
            if (SelectTable(2) == false) return false; // 选择表

            if (i2c.TWI_ReadPage(0xA2, 0xA0, checkbyte_a0, 1) != 1) return false; // 未初始化为 0x00
            if (i2c.TWI_ReadPage(0xA2, 0xC0, checkbyte_c0, 1) != 1) return false; // bit0 = 1 EEPROM正常

            // 判断模块是否初始化过
            if (checkbyte_a0[0] != 0x00)
            {
                // GN25L95已经初始化过 // 写入调试密码 A9 'F' 'T' 'P'
                if (i2c.TWI_WritePage(0xa2, 0x7B, byte_debug_pwd, 4) != 4) return false;
                if (i2c.TWI_ReadPage(0xA2, 0xC0, checkbyte_c0, 1) != 1) return false; // bit0 = 1 EEPROM正常
            }

            if ((checkbyte_c0[0] & 0x01) == 0x00) return false; // 0xC0 bit0 = 0 模块异常
            ...
            regcfg[48] = byte_debug_pwd[3];
            //
            regcfg[0xA4 - 0xA0] = 0x07; // 初始化： 关闭APC、MOD温度补偿
            //

            // 重要操作
            if (i2c.TWI_WriteByte(0xA2, 0xC7, 0x00) == false) return false; // 配置操作EEPROM
            //

            if (i2c.TWI_WritePage(0xA2, 0xA0, regcfg, 62) != 62) return false;
            Thread.Sleep(10);
            if (i2c.TWI_ReadPage(0xA2, 0xA0, regcfg_read, 62) != 62) return false;
            ...
            //
            if (i2c.TWI_WritePage(0xA2, 0xF8, awflag_en, 8) != 8) return false;
            if (i2c.TWI_ReadPage(0xA2, 0xF8, awflag_en_read, 8) != 8) return false;
            ...
            // 校验数据是否正确
            if (Bit.ByteEquals(regcfg, regcfg_read) && Bit.ByteEquals(awflag_en_read, awflag_en))
            {
                return true; // 初始化成功
            }
            else
        ...
        public bool TxTempLookupTableCtrl(bool enable)
        {
            ...
            if (enable)
            {
                if (i2c.TWI_WriteByte(0xa2, 0xA4, register[0xA4 - 0xA0]) == false) return false; // 按照数据库配置 开启温补
                if (i2c.TWI_WriteByte(0xa2, 0xC4, register[0xC4 - 0xA0]) == false) return false; // WRITE_ACCESS Registers 写入数据库配置
            }
            else
            {
                if (i2c.TWI_WriteByte(0xA2, 0xA4, 7) == false) return false; // 关闭温补
                if (i2c.TWI_WriteByte(0xA2, 0xC4, 19) == false) return false; // WRITE_ACCESS Registers 允许写入告警门限
            }
            //
            return true;
        }
        ...
        {
            return i2c.TWI_WriteByte(0xa2, 0x7F, tbl);
        }
