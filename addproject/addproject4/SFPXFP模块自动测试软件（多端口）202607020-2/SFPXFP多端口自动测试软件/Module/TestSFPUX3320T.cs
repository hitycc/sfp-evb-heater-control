...
{
    public class SFPUX3320T : ModuleTest
    {
        EVB i2c;
        private int _slot;
        ...
        public void Init(EVB i2c, byte Dut)
        {
            ...
            Dut_num = Dut;
            _slot = Dut;
            //MessageBox.Show("窗体1"+_slot.ToString());
        ...
        public bool GetDDMAnalogValues()
        {
            ...
            if (TWI_ReadPage(0xa2, 96, readbuffer, 10) == 10)
            {
                ...
                // 未初始化模块使用
                if ((GlobalVarFun.testType == "firstTest") && (TestResult.vccDDM < 1.0f))
                {
                    TestResult.vccDDM = 2.3f; // 2.3V 初测初始化
                }
        ...
        public bool WriteRxCalData()
        {
            ...
            try
            {
                ...
                //{
                //    //MessageBox.Show("I2C通讯有问题");
                //    return false;
                //}
                //Thread.Sleep(200); // 延时
                //if (TWI_ReadPage(0xA2, 0xC0, readByte, 16) != 16)
                //{
                //    //MessageBox.Show("I2C通讯有问题1");
                //    return false;
                //}
            ...
            //for(int i = 0;i<16;i++)
            //{
            //    MessageBox.Show("writeByte数组数据为：" + writeByte[i].ToString());

            //}
            //for (int i = 0; i < 16; i++)
            //{
            //    MessageBox.Show("readByte数组数据为：" + readByte[i].ToString());
        ...
        public bool SaveTxDataAfterDebug()
        {
            ...
            I2C_WriteSplit(0xA2, 0x60, A2Lower60h, 24);
            //Thread.Sleep(10);

            //if (TWI_WritePage(0xa2, 0x00, A2hLow96, 96) != 96) return false;
            if (!I2C_WriteSplit(0xA2, 0x00, A2hLow96, 96))
            {
                return false;
            }
            //Thread.Sleep(10);
            ...
            if (!I2C_ReadSplit(0xA2, 0x00, r_A2hLow96, 96))
            {
                return false;
            }
            ...
            if (!I2C_WriteSplit(0xA2, 0x80, w_biasLUT, 105))
            {
                return false;
            }
            ...
            if (!I2C_ReadSplit(0xA2, 0x80, r_biasLUT, 105))
            {
                return false;
            }
            ...
            if (!I2C_WriteSplit(0xA2, 0x80, w_modLUT, 105))
            {
                return false;
            }
            ...
            if (!I2C_ReadSplit(0xA2, 0x80, r_modLUT, 105))
            {
                return false;
            }
            ...
            if (!I2C_ReadSplit(0xA2, 0x80, r_apdLUT, 105))
            {
                return false;
            }
        ...
        public bool SelectTable(int tbl)
        {
            ...
            r_val[0] = 0xFF;
            if (TWI_ReadPage(0xA2, 0x7F, r_val, 1) != 1)
            {
                //MessageBox.Show("111");
                return false;
            }
            ...
            if (TWI_WriteByte(0xA2, 0x7F, tbl) == false)
            {
                //MessageBox.Show("222");
                return false;
            }
            ...
            if (r_val[0] != (byte)tbl)
            {
                //MessageBox.Show("r_val[0]:"+ r_val[0].ToString());
                //MessageBox.Show("tbl:" + tbl.ToString());
                //MessageBox.Show("444");
                return false;
            }
            return true;
        ...
        private bool GetEEPROMcheckSumDB()
        {
            ...
            SelectTable(4);
            //TWI_ReadPage(0xA2, 0x80, r_biasLUT, 105);
            I2C_ReadSplit(0xA2, 0x80, r_biasLUT, 105);

            SelectTable(5);
            //TWI_ReadPage(0xA2, 0x80, r_modLUT, 105);
            I2C_ReadSplit(0xA2, 0x80, r_modLUT, 105);

            SelectTable(6);
            //TWI_ReadPage(0xA2, 0x80, r_apdLUT, 105);
            I2C_ReadSplit(0xA2, 0x80, r_apdLUT, 105);

            SelectTable(3);
            //TWI_ReadPage(0xa2, 0x80, r_regTbl3, 93);
            I2C_ReadSplit(0xA2, 0x80, r_regTbl3, 93);
