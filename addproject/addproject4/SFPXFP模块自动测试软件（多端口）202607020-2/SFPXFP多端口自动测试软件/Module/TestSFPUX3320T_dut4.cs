...
{
    public class SFPUX3320T_Dut4 : ModuleTest
    {
        EVB i2c;
        private int _slot;
        ...
        public void Init(EVB i2c, byte Dut)
        {
            ...
            Dut_num = Dut;
            _slot = Dut;
            //MessageBox.Show("窗体4" + _slot.ToString());
            // 0x00:线性计算法 apc-->uw & bias   0x11: 普通二分法 apc-->dBm   22:差值二分法 apc-->uW
            GlobalVarFun.txpwr_debug_method = 0x11;

            TestResult4.flash_data_len = 1024; // 256+256+128+128+128+128=1024  必须<=1024
        ...
        public bool GetDDMAnalogValues()
        {
            ...
            if (TWI_ReadPage(0xa2, 96, readbuffer, 10) == 10)
            {
                ...
                TestResult4.tempDDM = (j + readbuffer[1] * (1 / 256.0f));
                TestResult4.vccDDM = ((readbuffer[2] * 256 + readbuffer[3]) / 10000.0f);
                // 未初始化模块使用
                if ((GlobalVarFun.testType == "firstTest") && (TestResult4.vccDDM < 1.0f))
                {
                    TestResult4.vccDDM = 2.3f; // 2.3V 初测初始化
                }
                TestResult4.txBiasDDM = ((readbuffer[4] * 256 + readbuffer[5]) / 500.0f);
                //
                ...
                if (readbuffer[8] == 0x00 && readbuffer[9] == 0x00)
                    readbuffer[9] = 0x01;
                //
                TestResult4.txPowerDDM = (float)(10 * Math.Log10((readbuffer[6] * 256 + readbuffer[7]) / 10000.0));
                TestResult4.rxPowerDDM = (float)(10 * Math.Log10((readbuffer[8] * 256 + readbuffer[9]) / 10000.0));
            }
            else
            {
                TestResult4.tempDDM = 0;
                TestResult4.vccDDM = 0;
                TestResult4.txBiasDDM = 0;
                TestResult4.txPowerDDM = -100;
                TestResult4.rxPowerDDM = -100;
                return false;
            }
        ...
        public bool GetDDMThresholds()
        {
            ...
            TestResult4.tempHA = (float)(i + readbuffer[1] * 1.0 / 256.0);
            i = Convert.ToInt32((sbyte)readbuffer[2]);
            TestResult4.tempLA = (float)(i + readbuffer[3] * 1.0 / 256.0);
            i = Convert.ToInt32((sbyte)readbuffer[4]);
            TestResult4.tempHW = (float)(i + readbuffer[5] * 1.0 / 256.0);
            i = Convert.ToInt32((sbyte)readbuffer[6]);
            TestResult4.tempLW = (float)(i + readbuffer[7] * 1.0 / 256.0);

            TestResult4.vccHA = (float)((readbuffer[8] * 256 + readbuffer[9]) / 10000.0);
            TestResult4.vccLA = (float)((readbuffer[10] * 256 + readbuffer[11]) / 10000.0);
            TestResult4.vccHW = (float)((readbuffer[12] * 256 + readbuffer[13]) / 10000.0);
            TestResult4.vccLW = (float)((readbuffer[14] * 256 + readbuffer[15]) / 10000.0);

            TestResult4.txBiasHA = (float)((readbuffer[16] * 256 + readbuffer[17]) / 500.0);
            TestResult4.txBiasLA = (float)((readbuffer[18] * 256 + readbuffer[19]) / 500.0);
            TestResult4.txBiasHW = (float)((readbuffer[20] * 256 + readbuffer[21]) / 500.0);
            TestResult4.txBiasLW = (float)((readbuffer[22] * 256 + readbuffer[23]) / 500.0);

            TestResult4.txPowerHA = (float)(10 * Math.Log10((readbuffer[24] * 256 + readbuffer[25]) / 10000.0));
            TestResult4.txPowerLA = (float)(10 * Math.Log10((readbuffer[26] * 256 + readbuffer[27]) / 10000.0));
            TestResult4.txPowerHW = (float)(10 * Math.Log10((readbuffer[28] * 256 + readbuffer[29]) / 10000.0));
            TestResult4.txPowerLW = (float)(10 * Math.Log10((readbuffer[30] * 256 + readbuffer[31]) / 10000.0));

            TestResult4.rxPowerHA = (float)(10 * Math.Log10((readbuffer[32] * 256 + readbuffer[33]) / 10000.0));
            TestResult4.rxPowerLA = (float)(10 * Math.Log10((readbuffer[34] * 256 + readbuffer[35]) / 10000.0));
            TestResult4.rxPowerHW = (float)(10 * Math.Log10((readbuffer[36] * 256 + readbuffer[37]) / 10000.0));
            TestResult4.rxPowerLW = (float)(10 * Math.Log10((readbuffer[38] * 256 + readbuffer[39]) / 10000.0));
            //
            return true;
        }
        ...
        public bool GetDDMFlagsInterrupt()
        {
            ...
            TestResult4.tempHA_flag = Bit.GetBit(readbuffer[0], 7);
            TestResult4.tempLA_flag = Bit.GetBit(readbuffer[0], 6);
            TestResult4.vccHA_flag = Bit.GetBit(readbuffer[0], 5);
            TestResult4.vccLA_flag = Bit.GetBit(readbuffer[0], 4);
            TestResult4.txBiasHA_flag = Bit.GetBit(readbuffer[0], 3);
            TestResult4.txBiasLA_flag = Bit.GetBit(readbuffer[0], 2);
            TestResult4.txPwrHA_flag = Bit.GetBit(readbuffer[0], 1);
            TestResult4.txPwrLA_flag = Bit.GetBit(readbuffer[0], 0);
            TestResult4.rxPwrHA_flag = Bit.GetBit(readbuffer[1], 7);
            TestResult4.rxPwrLA_flag = Bit.GetBit(readbuffer[1], 6);

            TestResult4.tempHW_flag = Bit.GetBit(readbuffer[4], 7);
            TestResult4.tempLW_flag = Bit.GetBit(readbuffer[4], 6);
            TestResult4.vccHW_flag = Bit.GetBit(readbuffer[4], 5);
            TestResult4.vccLW_flag = Bit.GetBit(readbuffer[4], 4);
            TestResult4.txBiasHW_flag = Bit.GetBit(readbuffer[4], 3);
            TestResult4.txBiasLW_flag = Bit.GetBit(readbuffer[4], 2);
            TestResult4.txPwrHW_flag = Bit.GetBit(readbuffer[4], 1);
            TestResult4.txPwrLW_flag = Bit.GetBit(readbuffer[4], 0);
            TestResult4.rxPwrHW_flag = Bit.GetBit(readbuffer[5], 7);
            TestResult4.rxPwrLW_flag = Bit.GetBit(readbuffer[5], 6);
        ...
        public bool GetFlashInfo()
        {
            ...
            for (i = 0; i < 256; i++)
            {
                TestResult4.flash_data[i] = readbuffer[i];
            }
            ...
            for (i = 0; i < 256; i++)
            {
                TestResult4.flash_data[i + 256] = readbuffer[i];
            }

            TestResult4.sn = System.Text.Encoding.ASCII.GetString(TestResult4.flash_data, 68, 16);
            TestResult4.pn = System.Text.Encoding.ASCII.GetString(TestResult4.flash_data, 40, 16);
            TestResult4.vn = System.Text.Encoding.ASCII.GetString(TestResult4.flash_data, 20, 16);
            TestResult4.date = System.Text.Encoding.ASCII.GetString(TestResult4.flash_data, 84, 8);
            //TestResult4.sn.TrimEnd();
            //TestResult4.pn.TrimEnd();
            //TestResult4.vn.TrimEnd();
            //TestResult4.date.TrimEnd();
        ...
        public bool GetFlashInfoDebug()
        {
            ...
            for (i = 0; i < 128; i++)
            {
                TestResult4.flash_data[i + 512] = readbuffer[i];
            }
            ...
            for (i = 0; i < 128; i++)
            {
                TestResult4.flash_data[i + 640] = readbuffer[i];
            }
            ...
            for (i = 0; i < 128; i++)
            {
                TestResult4.flash_data[i + 768] = readbuffer[i];
            }
            ...
            for (i = 0; i < 128; i++)
            {
                TestResult4.flash_data[i + 896] = readbuffer[i];
            }

            // 更新 Fsn  飞思卓产品内部流水号  0xF0-F4 table04 0xF0-0x80=112
            UInt64 iFsn = 0;
            iFsn += TestResult4.flash_data[640 + 112 + 0]; // 0x00 = 1
            iFsn <<= 8;
            iFsn += TestResult4.flash_data[640 + 112 + 1]; // 0x01 = 2
            iFsn <<= 8;
            iFsn += TestResult4.flash_data[640 + 112 + 2]; // 0x02 = 3
            iFsn <<= 8;
            iFsn += TestResult4.flash_data[640 + 112 + 3]; // 0x03 = 4
            iFsn <<= 8;
            iFsn += TestResult4.flash_data[640 + 112 + 4]; // 0x04 = 5

            if (iFsn > TestResult4.max_Fsn)
            {
                iFsn = TestResult4.max_Fsn;
            }
            TestResult4.fibertop_sn = iFsn.ToString("D12");
            //TestResult4.fibertop_sn = System.Text.Encoding.ASCII.GetString(TestResult4.flash_data, 640 + 0, 10); //0xF0-FA  table04
            //TestResult4.fibertop_sn.TrimEnd();
            //TestResult4.fibertop_sn = TestResult4.sn; //test
        ...
        public bool CheckThresholdsInfo(ref string errMsg)
        {
            ...
            for (i = 0; i < 56; i++)
            {
                threshold_read[i] = TestResult4.flash_data[i + 256];
            }

            for (i = 0; i < 39; i++)
            {
                ex_cal_read[i] = TestResult4.flash_data[i + 256 + 56];
            }
            ...
            for (i = 0; i < 95; i++)
            {
                check_sum += TestResult4.flash_data[i + 256];
            }

            // check sum errro
            if (check_sum != TestResult4.flash_data[95 + 256])
            {
        ...
        public bool CheckModuleFlashInfo(ref string errMsg)
        {
            ...
            for (i = 0; i < 93; i++)
            {
                ...
                r_regcfgT3[i] = TestResult4.flash_data[i + 512];
            }
            //regcfgT3[0xA2 - 0x80] = register[0xA2 - 0x80];
            //r_regcfgT3[0xA2 - 0x80] = TestResult4.flash_data[0xA2 - 0x80 + 512];
            for (i = 0; i < 105; i++)
            {
                r_biasLUT[i] = TestResult4.flash_data[i + 640];
                r_modLUT[i] = TestResult4.flash_data[i + 768];
                r_apdLUT[i] = TestResult4.flash_data[i + 896];
            }
        ...
        public bool GetTypeDebugInfoFromAccessdb()
        {
            ...
            try
            {
                ...
                dbconnectionstr = string.Format("select TxPowerSetPoint,TxERSetPoint,TxER_MIN,TxER_MAX,Bias_MAX,Bias_MIN,TxPower_MIN,TxPower_MAX,TxCr_MIN,TxCr_MAX,TxJt_MAX,Sensitivity,RxALos,RxDLos,RxOverLoad,"
                                             + @"RxPwrCheck_1,RxPwrCheck_2,RxPwrCheck_3,RxPwrCheck_4,RxPwrCheck_5,APD_Name,APCmin,APCmax,MODmin,MODmax,LOSmin,LOSmax,MaskName,MaskMargin,
                                              ...
                                              Test_25GALG,Test_COB_LD,Test_EML,Test_APD,Test_CopperSFP,Test_CDRDis,Test_SchemeDis,Test_Init,delay_PSSBERT,Rosa_PIN,Test_EyeSave
                                                from ModuleType where Type = '{0}'", TestResult4.fibertop_pn);
                ...
                TestResult4.mask_name = dbset.Tables["ModuleType"].Rows[0]["MaskName"].ToString().Trim();
                str = dbset.Tables["ModuleType"].Rows[0]["MaskMargin"].ToString().Trim();
                if (!string.IsNullOrEmpty(str))
                {
                    TestResult4.mask_margin = Convert.ToUInt16(str);
                }
                ...
                {
                    dbconnectionstr = string.Format("select register,{1},{2},{3} from [{0}]", TestResult4.fibertop_pn, AW_Threshold_Field, Apc_LUT_Field, Mod_LUT_Field);
                }
                else
                {
                    dbconnectionstr = string.Format("select register,{1},{2},{3},{4} from [{0}]", TestResult4.fibertop_pn, AW_Threshold_Field, Apc_LUT_Field, Mod_LUT_Field, Apd_LUT_Field);
                }

                dbcommand = new OleDbCommand(dbconnectionstr, dbconnect);
                dbadapter = new OleDbDataAdapter(dbcommand);
                dbset = new DataSet();
                dbadapter.Fill(dbset, TestResult4.fibertop_pn);
                //
                for (i = 0; i < 40; i++)
                {
                    threshold[i] = Convert.ToByte(dbset.Tables[TestResult4.fibertop_pn].Rows[i][AW_Threshold_Field]);
                }

                for (i = 0; i < 93; i++)
                {
                    register[i] = Convert.ToByte(dbset.Tables[TestResult4.fibertop_pn].Rows[i]["register"]);
                }

                for (i = 0; i < 84; i++)
                {
                    biaslut[i] = Convert.ToByte(dbset.Tables[TestResult4.fibertop_pn].Rows[i][Apc_LUT_Field]);
                }

                for (i = 0; i < 84; i++)
                {
                    modlut[i] = Convert.ToUInt16(dbset.Tables[TestResult4.fibertop_pn].Rows[i][Mod_LUT_Field]);
                }
                ...
                if (string.IsNullOrEmpty(TestSet.apdName) == false) // APD
                {
                    for (i = 0; i < 84; i++)
                    {
                        apdlut[i] = Convert.ToByte(dbset.Tables[TestResult4.fibertop_pn].Rows[i][Apd_LUT_Field]);
                    }
                }
        ...
        public bool WriteTxCalData()
        {
            ...
            TestResult4.txPwrCal_k = ((float)((Math.Pow(10, TestResult4.txPower / 10.0) * 10000.0) / ADC));
            TestResult4.txPwrCal_b = 0;

            float fk = TestResult4.txPwrCal_k;
            if (fk < 0) fk = 0;
            if (fk > 255) fk = 255;
            writeByte[0] = (byte)fk;
        ...
        public bool WriteRxCalData()
        {
            ...
            {
                //
                b = TestResult4.rxPwrCal_b[0];
                k = TestResult4.rxPwrCal_k[0];
                //
                ...
                //
                b = TestResult4.rxPwrCal_b[1];
                k = TestResult4.rxPwrCal_k[1];
                ...
                //
                b = TestResult4.rxPwrCal_b[2];
                k = TestResult4.rxPwrCal_k[2];
                ...
                writeByte[12] = (byte)(TestResult4.rxAdcCal[1] >> 8);
                writeByte[13] = (byte)(TestResult4.rxAdcCal[1] & 0xFF);
                writeByte[14] = (byte)(TestResult4.rxAdcCal[2] >> 8);
                writeByte[15] = (byte)(TestResult4.rxAdcCal[2] & 0xFF);
        ...
        public bool SaveTxDataAfterDebug()
        {
            ...
            //2021.4.25-5.9
            float f_bias = TestResult4.txBiasDDM; //1mA
            ...
            //delta_bias = 0; //TestResult4.txapcVal - biaslut[temp_index];
            delta_mod = TestResult4.txmodVal - modlut[temp_index];

            if (modlut[temp_index] == 0) return false; // 消光比补偿表不能为0

            // 计算比例系数
            float fk = TestResult4.txmodVal;
            ...
            for (i = 0; i < 96; i++)
            {
                TestResult4.flash_data[i + 256] = r_A2hLow96[i]; //
            }
            //
        ...
        public bool TxTempLookupTableCtrl(bool enable)
        {
            ...
            if (enable)
            {
                ...
                if (TestResult4.fibertop_pn.Contains("DCP") || TestResult4.fibertop_pn.Contains("DIP"))//APDLUT
                {
                    //byte value = 0x00;
