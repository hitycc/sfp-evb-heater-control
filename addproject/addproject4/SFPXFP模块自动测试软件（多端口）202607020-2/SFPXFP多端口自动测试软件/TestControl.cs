...
namespace SFPXFP自动测试软件多端口
{
    ...
    public class TestControl
    {
        ...
        public bool CulRxCalPar()
        {
            ...
            switch (Dut)
            {
                ...
                case 2:
                    ...
                    else
                    {
                        ...
                        //MessageBox.Show("x[1]:" + x[1].ToString());
                        //MessageBox.Show("x[2]:" + x[2].ToString());
                        //MessageBox.Show("y[0]:" + y[0].ToString());
                        //MessageBox.Show("y[1]:" + y[1].ToString());
                        //MessageBox.Show("y[2]:" + y[2].ToString());
            ...
            //MessageBox.Show("a[1]:" + a[1].ToString());
        ...
        private async Task<bool> RxPwrDDMAutoCal_Async()
        {
            ...
            otp12.VOA_SetApMode(Dut, "ABSolute");
            //打开输出光路（确保光路上有输出）
            //otp12.VOA_SetOutputState(VoaChannel, "ON");//因为在SetupFrm中已经设置了打开光路操作
            //设置TXSFP光源1
            await Task.Delay(waittimes);
            switch (Dut)
            {
            ...
            rxAdc[0] = test.GetRxADC();
            //MessageBox.Show("设置完光源1后rxAdc[0]："+ rxAdc[0].ToString());
            retutntxrxresult.RxddmPowers[0] = rxAdc[0];
            ...
            rxAdc[1] = test.GetRxADC();
            //MessageBox.Show("设置完光源2后rxAdc[1]：" + rxAdc[1].ToString());
            retutntxrxresult.RxddmPowers[1] = rxAdc[1];
            //retutntxrxresult.RxRealPowers[1] = TestSet2.rxPwr_Cal[1];
            result.message = "设置TXSFP光源2：Success" + " AttVal" + DOA.rxCalAtt[1].ToString() + " rxAdc:" + rxAdc[1].ToString();
            ModListBoxShow(this, result);
            ...
            rxAdc[2] = test.GetRxADC();
            retutntxrxresult.RxddmPowers[2] = rxAdc[2];
            //MessageBox.Show("设置完光源3后rxAdc[2]：" + rxAdc[2].ToString());
            result.message = "设置TXSFP光源3：Success" + " AttVal" + DOA2.rxCalAtt[2].ToString() + " rxAdc:" + rxAdc[2].ToString();
            ModListBoxShow(this, result);
            await Task.Delay(waittimes);
            if (GlobalVarFun.setup.rx_apd_cal) // APD 检查后面2个点
            ...
            rxAdc[5] += 3; //加大 预防跳动问题
            retutntxrxresult.RxddmPowers[5] = rxAdc[5];
            //MessageBox.Show("设置无光状态光源后rxAdc[5]：" + rxAdc[5].ToString());
            result.message = "设置TXSFP光源 为无光状态：Success" + " AttVal" + "60" + " rxAdc:" + rxAdc[5].ToString();
        ...
        private async Task<bool> RxSenBitErrorCheck_Async()
        {
            ...
            await Task.Delay(100); // 等待500ms让硬件稳定
        ...
        // Synchronous ERM read used by the synchronous extinction-ratio tuning path.
        private bool Get_ERatio_ERM(int dutId, bool autoScale)
        {
            float tx_er = 0;
            dcaSemaphore.Wait();
            try
            {
                for (int i = 0; i < 5; i++)
                {
                    string erData = otp12.ERM_ReadERData(dutId); //这里也要用传入dutId
                ...
                {
                    Thread.Sleep(500);
                    string erData = otp12.ERM_ReadERData(dutId);
                    if (!string.IsNullOrEmpty(erData))
                    {
                        string[] parts = erData.Split(',');
                        if (parts.Length >= 2)
            ...
            catch (Exception exp)
            {
                ...
                switch (dutId)
                {
                    case 1: TestResult.txEr = 0; break;
                    case 2: TestResult2.txEr = 0; break;
                    case 3: TestResult3.txEr = 0; break;
                    case 4: TestResult4.txEr = 0; break;
                }
                return false;
            }
            finally
        ...
        private bool AutoSetTxPower_MethodB()
        {
            ...
            pwr_maxErr = 0.7f; // 0.35dB  二分法查找精度
            ...
            //MessageBox.Show("刚进调试时min=" + min.ToString());
            //MessageBox.Show("刚进调试时max=" + max.ToString());
            //MessageBox.Show("刚进调试时TxPwr_Min=" + TxPwr_Min.ToString());
            //MessageBox.Show("刚进调试时TxPwr_Max=" + TxPwr_Max.ToString());
            //MessageBox.Show("刚进调试时Bias_Min=" + Bias_Min.ToString());
            //MessageBox.Show("刚进调试时Bias_Max=" + Bias_Max.ToString());

            //MessageBox.Show("发射调试目标光功率=" + TestSet.txPwr_target.ToString());//-5.5
            ...
            do
            {
                ...
                apc = (UInt16)((min + max) / 2);
                //MessageBox.Show("刚进调试时APC=" + apc.ToString());
                ...
                switch (Dut)
                {
                    ...
                    case 2:
                        ...
                        float.TryParse(txpower, out pwr);
                        pwr += meter_err;
                        //MessageBox.Show("pwr = "+ pwr.ToString());
                        TestResult2.txPower = pwr;
                        if (pwr < -40)
                        {
                            for (int i = 0; i < 3; i++)
                        ...
                        AddTestLog("txpwer2:" + TestResult2.txPower.ToString() + " APC:" + apc.ToString()  + " meter_err:" + meter_err.ToString());
                        break;
                    case 3:
                        txpower = otp12.OPM_ReadPower(opmCh);
                        float.TryParse(txpower, out pwr);
                ...
                //MessageBox.Show("与目标值的差值result_err = " + result_err.ToString());
                //
                if (result_err > 0)
                {
                    max = (UInt16)(apc - 1);
        ...
        private bool TxErAutoSet_Sync()
        {
            if (GlobalVarFun.txpwr_debug_method == 0x33)
            {
                AddTestLog("txpwr_debug_method:0x33");
                // The 0x33 algorithm is not used by the current 0x11 flow. Keep
                // its existing implementation available without changing behavior.
                return AutoSetTxEr_MethodE_Sync();
            }
            else
            {
                AddTestLog("txpwr_debug_method:其他，AutoSetTxEr_MethodA");
                return AutoSetTxEr_MethodA_Sync(Dut);
            }
        }
        #endregion
        ...
        #region  // 待测模块消光比自动调试   普通二分法  同步ERM
        private bool AutoSetTxEr_MethodA_Sync(int dutId)
        {
            UInt16 min = (UInt16)TestSet.txmod_Min;
            UInt16 max = (UInt16)TestSet.txmod_Max;
            UInt16 mod = 0;
            ...
            AddTestLog($"[DUT{dutId}] erValMaxErr:{erValMaxErr} er_target:{er_target} MOdmin:{min} Modmax:{max}");

            try
            {
                //===== 只在操作OTP12仪器的时候拿锁，不要包住整个do‑while循环！=====
                switchSemaphore.Wait();
                try
                {
                    //otp12.SW_SetRouteForModule(dutId, true);
                    otp12.SetSlot("06");
                    otp12.ERM_SetRate(dutId, "1.25G");
                    Thread.Sleep(1000);
                }
                finally
                {
                ...
                do
                {
                    ...
                    //写调制偏置寄存器，I2C操作，这个是每个DUT独立硬件，不需要switchSemaphore锁
                    if (test.SetTxModBias(mod) == false)
                        return false;

                    //读OTP12 ERM消光比，内部使用信号量保护共享仪器
                    bool res = Get_ERatio_ERM(dutId, false);
                    if (res == false)
                        return false;
                    ...
                    {
                        case 1:
                            currentER = TestResult.txEr;
                            if (currentER > 99)
                            {
                                res = Get_ERatio_ERM(dutId, false);
                                if (res == false) return false;
                                currentER = TestResult.txEr;
                            }
                            break;
                        case 2:
                            currentER = TestResult2.txEr;
                            if (currentER > 99)
                            {
                                res = Get_ERatio_ERM(dutId, false);
                                if (res == false) return false;
                    ...
                    result_err = currentER - er_target;
                    AddTestLog($"[DUT{dutId}] mod:{mod} txEr:{currentER} txEr_target:{er_target}");
                ...
                switch (dutId)
                {
                    ...
                    case 2:
                        TestResult2.txErErr = result_err;
                        retutntxrxresult.TxerResultShow = TestSet.txEr_target.ToString("F1") + "/" + TestResult2.txEr.ToString("F1");
                        break;
                    case 3:
                        TestResult3.txErErr = result_err;
                        retutntxrxresult.TxerResultShow = TestSet.txEr_target.ToString("F1") + "/" + TestResult3.txEr.ToString("F1");
                        break;
                    case 4:
                        TestResult4.txErErr = result_err;
                        retutntxrxresult.TxerResultShow = TestSet.txEr_target.ToString("F1") + "/" + TestResult4.txEr.ToString("F1");
                        break;
                }

                if (Math.Abs(result_err) <= erValMaxErr)
                {
                    return true;
                }
                else
                {
                    ...
                    {
                        if (result_err > 0) retutntxrxresult.ErrorMessage += "ER: The extinction ratio is large;";
                        if (result_err < 0) retutntxrxresult.ErrorMessage += "ER: The extinction ratio is small;";
                    }
                    return false;
                }
            }
        ...
        #region //待测模块消光比自动调试  普通二分法 同步
        private bool AutoSetTxEr_MethodE_Sync()
        {
            UInt16 apc_min = 0;
            UInt16 apc_max = 0;
            UInt16 mod_min = 0;
