...
namespace FibertopTest_Common
{
    ...
    public class CP2112:I2C
    {
        ...
        public bool setModuleDis(bool dis)
        {
            if (dis)
            {              
                CP2112.HidSmbus_WriteLatch(device, 0x04, 0x04);
            }
            else
            {
                CP2112.HidSmbus_WriteLatch(device, 0x00, 0x04);
            }
            return true;
        }
        ...
        {
            byte latchValue = 0x00;
            CP2112.HidSmbus_ReadLatch(device, ref latchValue);
            latchValue &= 0x08;
            if (latchValue != 0)
            {
                return false;
