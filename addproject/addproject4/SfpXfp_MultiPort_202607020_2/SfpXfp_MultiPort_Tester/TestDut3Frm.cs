...
{
    public partial class TestDut3Frm : Form
    {
        TestControl testcontrol3 = new TestControl(TestSet3.Dut);
        public TestDut3Frm()
        {
            InitializeComponent();
            GlobalVarFun.mycontrol_dut3 = testcontrol3;
            //MessageBox.Show("创建端口3");
            GlobalVarFun.mycontrol_dut3.ModListBoxShow = EventListBoxShow;
        }
        ...
        SimpleLogger simpleLogger3 = new SimpleLogger("D:\\SFPXFPTesTLogDUT3.txt");
        ...
        {
            if (simpleLogger3 != null)
            {
                simpleLogger3.LogInfo(reuslt.message);
            }
        }

        private int _ddmReading3 = 0; // 重入保护
        private async void ShowModuleDdmInfo()
        {
            // 重入保护：如果上一次DDM读取还没完成，直接跳过本次，防止请求堆积
            if (Interlocked.Exchange(ref _ddmReading3, 1) == 1)
                return;
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    testcontrol3.Read_moduleInfo_Async();
                    testcontrol3.Converted_analog_values_Async();
                    testcontrol3.Read_AlarmWarn_Thresholds_Async();
                });
                // await之后自动回到UI线程，直接更新UI，无需BeginInvoke
                Temp_textBox.Text = (TestResult3.tempDDM).ToString("F2");
                Vcc_textBox.Text = (TestResult3.vccDDM).ToString("F2");
                Bias_textBox.Text = (TestResult3.txBiasDDM).ToString("F2");
                TxPWR_textBox.Text = (TestResult3.txPowerDDM).ToString("F2");
                RxPWR_textBox.Text = (TestResult3.rxPowerDDM).ToString("F2");
            }
            finally
            {
                Interlocked.Exchange(ref _ddmReading3, 0);
            }
        }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // 测试SQL数据连接情况
            //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            if (testcontrol3.sqlserver != null && !GlobalVarFun.sqlserver_ip.Contains("null"))
            {
                try
                {
                    //sqlconnection.Open();
                    testcontrol3.sqlserver.ServersOpen();
                    GlobalVarFun.sql_connect_status_3 = true; // 数据库连接正常
                }
                catch (Exception exp)
                {
                    GlobalVarFun.sql_connect_status_3 = false; // 数据库连接失败
                    MessageBox.Show(exp.Message);
                }
                finally
                {
                    testcontrol3.sqlserver.ServersClose();
                }
            }
            GlobalVarFun.sql_record_status_3 = GlobalVarFun.sql_connect_status_3;//更新记录状态和SQL连接状态一致 2018.5.19
            //TestResult3.tester_no = textBoxTester.Text;
        }
        ...
        private void start_button_Click(object sender, EventArgs e)
        {
            ...
            {
                if (TestSet3.wLength_target == 0)
                {
                    return;
                }
            }
            TestResult3.fibertop_pn = TestResult.fibertop_pn;
            

            // 判断调试参数范围设置是否正确
            if ((TestSet3.txapc_Max < TestSet3.txapc_Min) || (TestSet3.txmod_Max < TestSet3.txmod_Min) || (TestSet3.rxlos_Max < TestSet3.rxlos_Min))
            {
            ...
            if (autoTestCtrl == false)
            {
                ...
                if (GlobalVarFun.testDataIsOK3 == false)
                {
                    if (GlobalVarFun.Language == "Chinese")
                    {
        ...
        private async void timer1_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    {
                        typeCheckOk = testcontrol3.CheckTestTypeInfo();
                    }
                    else
                    {
                        TestResult3.chipIsOK = true;
                    }

                    if (typeCheckOk && TestResult3.fibertop_pn != "SFP-SM31TG-10DIU")
                    {
                        statusCheckOk = testcontrol3.ShowCheckModuleStatus();
                    }

                    if (autoTestCtrl && !moduleOnline)
                    {
                        debugPwd = GlobalVarFun.mycontrol_dut3.CheckDebugPWD();
                    }
                });
                ...
                if (TestResult3.vccDDM < 2.0)
                {
                   ...
                   /* SetLED(i2cok_pictureBox1, true);
                    ...
                    toolStripStatusLabel1.Text = "用做测试，端口3";
                    Startautoset_button.BackColor = Color.Orange;
                    return;
                }
                ...
                if ((GlobalVarFun.sql_connect_status_3 == true) && (GlobalVarFun.sql_record_status_3 == false))
                {
                    Startautoset_button.BackColor = Color.OrangeRed;
                    if (GlobalVarFun.Language == "Chinese")
                    {
            ...
            {
                // 定时器异常保护，避免崩溃
                System.Diagnostics.Debug.WriteLine($"timer1_Tick DUT3 error: {ex.Message}");
            }
            finally
            {
                Interlocked.Exchange(ref _tickProcessing, 0);
        ...
        private async void StartNewTestSequence()
        {
            ...
            TestResult3.test_status = 1;//开始测试
            //pnshow_textBox.Text = TestResult2.fibertop_pn;
           /* SetLED(i2cok_pictureBox1, false);
            SetLED(i2cok_pictureBox2, false);*/

            simpleLogger3.FileDelete();
            simpleLogger3 = new SimpleLogger("D:\\SFPXFPTesTLogDUT3.txt");
            moduleOnline = true;
            Update();
            // 模块DDM温度判断 0~40
            ...
            {
                if ((TestResult3.tempDDM > 40) || (TestResult3.tempDDM < 0))
                {
                    Startautoset_button.BackColor = Color.OrangeRed;
                    if (GlobalVarFun.Language == "Chinese")
                    {
            ...
            if (GlobalVarFun.testType != "firstTest")
            {
                if ((TestResult3.vccDDM > 3.45) || (TestResult3.vccDDM < 3.15))
                {
                    Startautoset_button.BackColor = Color.OrangeRed;
                    if (GlobalVarFun.Language == "Chinese")
                    {
        ...
        private async Task RunTestSequenceAsync(CancellationToken cancellationToken)
        {
            ...
            TestResult3.Test_ok = false;
            TestResult3.test_status = 1;
            if (GlobalVarFun.testType == "firstTest")
            {
                Task<bool> dut3first = testcontrol3.FirstTestProcessAsync(uiUpdateProgressForRxPwr as IProgress<ReturnTxRxResult>);
                bool firstTestResult = await dut3first;
                //bool firstTestResult = await  testcontrol2.FirstTestProcessAsync(uiUpdateProgressForRxPwr as IProgress<ReturnTxRxResult>);
                if (firstTestResult)
                {
                    TestResult3.Test_ok = true;
            ...
            {
                Task<bool> dut3final = testcontrol3.FinalTestProcessAsync(uiUpdateProgressForRxPwr as IProgress<ReturnTxRxResult>);
                bool finalTestResult = await dut3final;
        ...
        private void UpdateUIControls(ReturnTxRxResult result)
        {
            ...
            this.BeginInvoke(new MethodInvoker(() =>
            {
                ...
                if (TestResult3.Test_ok == false)
                {
                    if (TestResult3.test_status == 3)
                    {
                ...
                if (TestResult3.test_status == 1)
                {
                    Startautoset_button.BackColor = Color.White;
                }
                if (TestResult3.test_status == 0)
                {
                    Startautoset_button.BackColor = Color.Orange;
                }
            }));
