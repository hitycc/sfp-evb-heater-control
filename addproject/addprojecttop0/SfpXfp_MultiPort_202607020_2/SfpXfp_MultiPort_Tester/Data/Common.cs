...
namespace FibertopTest_Common
{
    public class GlobalVarFun
    {
        ...
        /// <summary>
        /// 模块层调试日志事件：参数为 (端口号slot, 日志消息)
        /// </summary>
        public static event Action<int, string> OnModuleDebugLog;
        public static void ModuleDebugLog(int slot, string msg)
        {
            OnModuleDebugLog?.Invoke(slot, $"[DUT{slot}] {DateTime.Now:HH:mm:ss.fff} {msg}");
        }
