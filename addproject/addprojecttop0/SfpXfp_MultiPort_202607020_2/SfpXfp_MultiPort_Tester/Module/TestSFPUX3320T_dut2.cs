...
namespace FibertopTest_Common
{
    public class SFPUX3320T_Dut2 : ModuleTest
    {
        ...
        public void Init(EVB i2c, byte Dut)
        {
            ...
            Dut_num = Dut;
            _slot = Dut;
            //MessageBox.Show("窗体2" + _slot.ToString());
        ...
        public bool CheckTestTypeInfo()
        {
            ...
            GlobalVarFun.ModuleDebugLog(_slot, "CheckTestTypeInfo[Dut2]: 开始检查模块...");

            //SelectTable(3);
            temp_val[0] = 0x03;
            int writeRet = TWI_WritePage(0xA2, 127, temp_val, 1);
            GlobalVarFun.ModuleDebugLog(_slot,
                $"CheckTestTypeInfo[Dut2]: 写A2:7F(选页3) 返回={writeRet} {(writeRet == 1 ? "OK" : "FAIL")}");
            if (writeRet != 1) //写入table3
            {
                GlobalVarFun.ModuleDebugLog(_slot, "CheckTestTypeInfo[Dut2]: 选页3失败，返回false");
                return false;
            }

            int readRet = TWI_ReadPage(0xA2, 0xF5, readChipID, 7);//读取ChipID
            string readIdStr = BitConverter.ToString(readChipID).Replace("-", " ");
            string expectIdStr = BitConverter.ToString(ux3320cID).Replace("-", " ");
            GlobalVarFun.ModuleDebugLog(_slot,
                $"CheckTestTypeInfo[Dut2]: 读A2:F5(ChipID) 返回{readRet}字节, 读到=[{readIdStr}], 期望=[{expectIdStr}]");
            if (Bit.ByteEquals(readChipID, ux3320cID) == false)
            {
                GlobalVarFun.ModuleDebugLog(_slot, "CheckTestTypeInfo[Dut2]: ChipID不匹配，返回false");
                // 恢复page0
                temp_val[0] = 0x00;
                TWI_WritePage(0xA2, 127, temp_val, 1);
                return false;
            }
            GlobalVarFun.ModuleDebugLog(_slot, "CheckTestTypeInfo[Dut2]: ChipID匹配UX33200 ✓");

            //终测检查E2PROM状态指示 F0h_Table03
            if (GlobalVarFun.testType == "finalTest")
            {
                int efRead = TWI_ReadPage(0xA2, 0xEF, temp_val, 2);
                GlobalVarFun.ModuleDebugLog(_slot,
                    $"CheckTestTypeInfo[Dut2]: 终测模式,读A2:EF 返回{efRead}字节, data[0]=0x{temp_val[0]:X2}, data[1]=0x{temp_val[1]:X2}, bit7-2=0x{(temp_val[1] & 0xFC):X2}");
                if (efRead != 2)
                {
                    GlobalVarFun.ModuleDebugLog(_slot, "CheckTestTypeInfo[Dut2]: 读A2:EF失败，返回false");
                    temp_val[0] = 0x00;
                    TWI_WritePage(0xA2, 127, temp_val, 1);
                    return false;
                }
                if ((temp_val[1] & 0xFC) != 0x00)
                {
                    GlobalVarFun.ModuleDebugLog(_slot, "CheckTestTypeInfo[Dut2]: E2PROM状态bit7-2非0，返回false");
                    temp_val[0] = 0x00;
                    TWI_WritePage(0xA2, 127, temp_val, 1);
                    return false;
                }
        ...
        //光模块出厂前需要校准"发射光功率"的读数，
        //使得模块内部上报的功率值和实际光功率计测到的一致。这个函数就是计算校准系数并写入模块芯片的寄存器中。
        //方法名意为"写发射端(Tx)校准(Cal)数据(Data)
        public bool WriteTxCalData()
        {
            byte[] writeByte = new byte[4]; // 准备写入芯片的4字节数据缓冲区
            byte[] readByte = new byte[4];  // 用来回读验证的4字节缓冲区
            //byte[] calpwr_uw = new byte[2];
            float ADC = 0;// 存储从芯片读取的ADC原始值
            // 选择表 03
            //这里准备了两个4字节数组，一个写、一个读，
            //是嵌入式开发里常见的"写后读校验"模式——写完数据后再读出来，
            //确认确实写进去了。
            if (SelectTable(3) == false) return false;
            // 计算校准参数
            ADC = GetTxADC();
