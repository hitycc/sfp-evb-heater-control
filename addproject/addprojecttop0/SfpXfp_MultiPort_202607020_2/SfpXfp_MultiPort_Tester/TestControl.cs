...
namespace SFPXFP自动测试软件多端口
{
    ...
    public class TestControl
    {
        ...
        public async Task<bool> RxLosAutoSet_Async()
        {
            ...
            // 3. 检查中点是否可以产生LOS
            //los_val = (min+max)/2，VOA仍在ALosAtt
            los_val = min;
            los_val += max;
            los_val /= 2;
            if (test.SetRxLos(los_val) == false) return false;
            ...
            // 4. 自动调试Los
            //线性搜索找阈值点（核心循环）
            //从min开始，步长2（接近max时步长1）递增los_val:
            //a) 写入当前los_val到模块
            //如果有告警(true) → 找到候选值了！
            //VOA切回DLosAtt（高光功率）验证去告警
            //去告警也OK(false) → ✅返回true 成功！
            //去告警失败 → ❌返回false 失败
            //如果无告警(false) → los_val太小，继续增加
            //等待waittimes让硬件稳定
            //los_val超过max仍没找到 → ❌返回false
            los_val = min;
            do
            {
                if (test.SetRxLos(los_val) == false) return false;
        ...
        //这是光模&#x5757;__&#x53D1;射（Tx）光功率自动调试的入口方法，目的是自动调节模块寄存器使发射光功率达到目标值。
        private async Task<bool> TxPowerAutoSet_Async()
        {
            //从测试配置中读取消光比调制电流（Mod）的最小值 txmod_Min
            UInt16 mod = TestSet.txmod_Min;
            //在此基础加10，作为初始调制值。这样做是为了给消光比留一点余量，避免调制值太小导致消光比不够
            mod += 10;
            //调用硬件接口将刚才计算的初始调制值（min+10）写入光模块寄存器
            //如果写入失败（I2C通信异常等），直接返回`false`表示调试失败
            //SetTxModBias是设置调制电流（Bias/Mod）的底层函数
            // 消光比调到 min + 10
            if (test.SetTxModBias(mod) == false) return false;
            //
            ...
            if (GlobalVarFun.txpwr_debug_method == 0x00)
            {
                AddTestLog("txpwr_debug_method:0x00");
                // return AutoSetTxPower_MethodA(); // 线性计算法 apc-->uw & bias
                //线性计算法：根据当前APC寄存器值与光功率(μW)的线性关系，直接计算目标APC值，同时调节Bias
            ...
            else if (GlobalVarFun.txpwr_debug_method == 0x11)
            {
                AddTestLog("txpwr_debug_method:0x11");
                //return AutoSetTxPower_MethodB(); // 普通二分法 apc-->dBm
                //普通二分法(dBm)：在APC寄存器范围内二分查找，以dBm为单位判断与目标值的偏差
            ...
            else if (GlobalVarFun.txpwr_debug_method == 0x22)
            {
                AddTestLog("txpwr_debug_method:0x22");
                //return AutoSetTxPower_MethodC(); // 差值二分法 apc-->uW
                //差值二分法(μW)：二分查找的变体，以μW（微瓦）线性功率为基准计算差值，比dBm更线性
            ...
            else if (GlobalVarFun.txpwr_debug_method == 0x33)
            {
                ...
                //return AutoSetTxPower_MethodD(); // 差值二分法 apc-->uW , 0.6倍bias
                //return AutoSetTxPower_MethodE(); // 普通二分法 apc-->dBm
                //普通二分法(dBm)：和MethodB类似，可能是优化版本（0.6倍Bias的版本被注释掉了）
                bool res = await AutoSetTxPower_MethodE_Async();
                return res;
            }
            //如果配置的方法值不是 0x00/0x11/0x22/0x33 中的任何一个
            //根据当前语言设置（中文/英文）写入对应的错误消息
            else
            {
                if (GlobalVarFun.Language == "Chinese")
                {
         ...
         IProgress<ReturnTxRxResult> progress)// 参数类型改为 IProgress<T>   
        {
            ...
            if (GlobalVarFun.setup.rx_test)
            {
                ...
                AddTestLog("RxLosAutoSet");// 记录测试日志
                Task<bool> rxlosauto = RxLosAutoSet_Async();// 启动异步校准
                res = await rxlosauto;// 等待完成
                if (res == false)
